...
list<const game_torrent *> Torrent_server::search_mat(const char mat)
{
    try
    {
        list<const game_torrent*> search;
        for(auto &it : torrents)
            if(dynamic_cast<const game_torrent*>(it)!=nullptr)
                search.push_back(dynamic_cast<const game_torrent*>(it));
